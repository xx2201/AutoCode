...
模型调用的最大输出预算。Agent 预留输出预算后，用 token 基础预算、准备 buffer 和硬边界
管理窗口，不再按 50%/70%/90% 裁剪历史。模型使用笔记工具保存状态，再调用 `new_context`；
程序在批次结束后无摘要切窗，硬边界和手动切窗使用同一流程。
新窗口提供恢复入口，笔记及原始消息按需读取，不再把全部 notes 常驻上下文。
...
| `notes.json` | 模型维护的虚拟笔记文件与来源 | 写入时原子保存；旧 task_notes.json 单向迁移且保留原件 |
