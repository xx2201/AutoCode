"""Telegram adapter for CoreCoder remote control."""
...
def _build_application(config: Config):
    ...
    app.add_handler(telegram["CommandHandler"]("approveall", _approve_all))
    ...
    app.add_handler(
        telegram["MessageHandler"](
            telegram["filters"].Regex(r"^/approve-all$"),
            _approve_all,
        )
    )
    app.add_handler(
...
def _validate_config(config: Config):
    if not config.model:
        raise SystemExit("No model configured. Set CORECODER_MODEL before starting Telegram control.")
    if not config.api_key:
        raise SystemExit("No API key configured. Set CORECODER_API_KEY before starting Telegram control.")
    if not config.telegram_bot_token:
        raise SystemExit("No Telegram bot token configured. Set CORECODER_TELEGRAM_BOT_TOKEN.")
    if not config.telegram_allowed_chat_ids:
        raise SystemExit(
            "No allowed Telegram chats configured. Set CORECODER_TELEGRAM_ALLOWED_CHATS to a comma-separated list."
        )
...
def _import_telegram():
    try:
        ...
        raise SystemExit(
            "Telegram support is optional. Install it with: pip install 'corecoder[telegram]'"
        ) from exc
    return {
...
async def _start(update, context):
    if not _is_allowed_chat(update, context):
        return
    text = (
        "CoreCoder Telegram control is ready.\n\n"
        "Commands:\n"
