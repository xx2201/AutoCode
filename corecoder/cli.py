...
def main():
    ...
    if not config.model:
        ...
        console.print(
            "Set `CORECODER_MODEL`, pass `-m/--model`, or configure the agent repo `.env`.\n"
    ...
    if not config.api_key:
        ...
        console.print(
            "Set `CORECODER_API_KEY` for the agent runtime.\n"
            "\nExamples:\n"
            "  # OpenAI-compatible runtime\n"
            "  export CORECODER_API_KEY=sk-...\n"
            "  export CORECODER_BASE_URL=https://api.openai.com/v1\n"
            "  export CORECODER_MODEL=gpt-4o\n"
            "\n"
            "  # DeepSeek\n"
            "  export CORECODER_API_KEY=sk-...\n"
            "  export CORECODER_BASE_URL=https://api.deepseek.com\n"
            "  export CORECODER_MODEL=deepseek-chat\n"
            "\n"
            "  # Ollama (local)\n"
            "  export CORECODER_API_KEY=ollama\n"
            "  export CORECODER_BASE_URL=http://localhost:11434/v1\n"
        ...
        if user_input == "/task":
            if agent.task_state is None:
                ...
                console.print(
                    ...
                    f"approve-all: [bold]{auto}[/bold]{pending}"
                )
            continue
        if user_input == "/todo":
            if agent.task_state is None:
...
def _prompt_approval(pending) -> str | bool:
    ...
    prompt = (
        ...
        f"{command_line}\n[y/N or /approve /approve-all /reject] "
    )
