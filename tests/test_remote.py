...
def test_remote_manager_persists_more_than_200_changed_files(tmp_path):
    llm = _FakeLLM([LLMResponse(content="finished")])
    manager = RemoteManager(_config(tmp_path), llm_factory=lambda: llm, tools=[])
    manager.submit(808, "create many files")
    changed_files = [
        {
            "path": f"generated/file-{index:03}.txt",
            "status": "untracked",
            "additions": 1,
            "deletions": 0,
        }
    ...
    manager.annotate_turn_changes(808, changed_files)

    messages = manager.conversation_messages(808)
    assert messages[0]["changed_files"] == changed_files
