...
def test_audit_logger_appends_events(tmp_path, monkeypatch):
    ...
    logger = AuditLogger()
    logger.handle("user_message", {"session_id": "session_audit", "task_id": "task_audit", "content_preview": "hello"})
    logger.handle("after_llm", {"session_id": "session_audit", "task_id": "task_audit", "step_index": 1, "prompt_tokens": 10, "completion_tokens": 5})
